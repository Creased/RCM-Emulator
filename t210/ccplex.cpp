...
// The bits ccplex_boot_cpu0() clears for CPU0 - CPURESET0, DBGRESET0,
// CORERESET0 and bit 30 - and NONCPURESET, which it clears first. The core
// runs once all five are clear.
#define  RST_CPU0_MASK         ((1u << 0) | (1u << 16) | (1u << 24) | (1u << 30))
...
struct Model {
  uint32_t rst_cpug = 0xFFFFFFFFu;   // everything held at power-on
...
const char *boot_blocked(EmuState *state) {
  ...
  if (!flow_ram_repair_done())
    return "fast-cluster RAM repair never requested (FLOW_CTLR_RAM_REPAIR)";
...
bool start_cpu0(EmuState *state) {
  ...
  // set of bytes. CPU0's engine is new on every release, so it cannot hold
  // stale translations of code the BPMP has rewritten since the last boot.
...
void ccplex_car_write(EmuState *state, uint32_t offset, uint32_t val) {
  switch (offset) {
  case CAR_CCLK_BURST_POLICY: m.cclk_burst = val; break;
  case CAR_PLLX_BASE:         m.pllx_base = val;  break;
  case CAR_CLK_OUT_ENB_V:     m.clk_enb_v = val;  break;
  case CAR_CLK_ENB_V_SET:     m.clk_enb_v |= val; break;
...
void ccplex_sb_write(EmuState *state, uint32_t offset, uint32_t val) {
  ...
  case SB_CSR:
    ...
    // non-secure writers; the BPMP is secure, so the vector stays writable
    // from here either way.
    m.sb_csr |= val;
    break;
  case SB_AA64_RESET_LOW:  m.sb_rst_lo = val; break;
  case SB_AA64_RESET_HIGH: m.sb_rst_hi = val; break;
  default: break;
  }
}
...
void ccplex_run(EmuState *state, uint64_t until_us) {
  ...
  // register model reads state->emu_usec, and must see CPU0's time, not the
  // BPMP's. Both are put back afterwards.
  ...
  uc_err err = uc_emu_start(m.uc, m.pc, 0, 0, 0);
